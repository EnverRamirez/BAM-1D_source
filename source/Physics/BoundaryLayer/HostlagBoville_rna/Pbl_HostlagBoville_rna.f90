MODULE Pbl_HostlagBoville_rna
 ...
 USE Parallelism, ONLY: &
  ...
  REAL(KIND=r8), ALLOCATABLE  :: delsig2(:)
  REAL(KIND=r8)               :: c0
...
CONTAINS
  ...
  SUBROUTINE vdinti_rna(ibMax,jbMax,plev, sig, delsig, sigml,pnats,pcnst)
    !----------------------------------------------------------------------- 
    ! 
    ! Purpose: 
    ...
    REAL(KIND=r8),    INTENT(IN) :: sig   (plev)
    REAL(KIND=r8),    INTENT(IN) :: delsig(plev)
    REAL(KIND=r8),    INTENT(IN) :: sigml (plev+1)
    ...
    ALLOCATE(delsig2(plev))               ;delsig2=0.0_r8
    ...
    DO k=plev,1,-1
       hypm(k) =  ps0*sig(k)
    END DO
    qmin(1) = 1.0e-12_r8        ! Minimum mixing ratio for moisture
    DO m=2,pcnst
       qmin(m) = 1.02-12_r8
    END DO
    qmincg(1) = 1.e-12_r8
...
!    !----------------------------------------------------------------------
  ...
  SUBROUTINE vdiff(bstar , &! INTENT(IN   ) :: 
    ...
    !
    CALL pbldif(thm     , &! INTENT(IN   ) ::  th(plond,plev)               ! potential temperature [K]
                ...
                TKEMYJ_HSBO, &! INTENT(OUT  ) ::  tke(plond,plev + 1)     ! eddy diffusivity 
                ...
                PBLH_HSBO , &! INTENT(OUT  ) ::  pblh(plond)               ! boundary-layer height [m]
                tpert   , &! INTENT(OUT  ) ::  tpert(plond)               ! convective temperature excess
                qpert   , &! INTENT(OUT  ) ::  qpert(plond)               ! convective humidity excess
                ...
                tsk     , &
                qsfc    , &
                rino    , &        ! bulk Richardson no. from level to ref lev
                psomc   , &
                obklen  , &
                phiminv , &
                phihinv , &
  ...
  SUBROUTINE virtem(t       ,q       ,zvir    ,tv , plond  ,plev ,plon )
    !-----------------------------------------------------------------------
    !
...
9000 FORMAT(' QNEG3 from ',a8,':m=',i3, &
  ...
  SUBROUTINE pbldif(th      , &! INTENT(IN   ) ::  th(plond,plev)          ! potential temperature [K]
...
1000   CONTINUE           ! end of level loop
     ...
     SUBROUTINE MY20_TURB (       &
      ...
      c=twodt*c0
       ...
       DO k = 1, plev-1
          DO i = 1, plond
             Pbl_ATemp(i,k)  = t0(k)*tm1 (i,(plev+1-k)) + t1(k)*tm1(i,(plev+1-k)-1)
          END DO
       END DO
       ...
       DO k = 1, plev
          DO i = 1, plond
             Pbl_PotTep(i,k)=sigkiv(k)*tm1 (i,(plev+1-k))*(1.0_r8+eps*qp1(i,(plev+1-k),1))
          END DO
       END DO
       !     
       ...
       DO k = 1, plev-1
          DO i = 1, plond
             ...
             !               SIN(colrad(i))**2
             !
             Pbl_Shear(i,k)=(con1(k))*Pbl_ITemp(i,k)*((um1(i,(plev+1-k))-um1(i,(plev+1-k)-1))**2+&
      ...
      k=1
      DO i = 1, plond
         Pbl_HgtLyI(i,k)=con0(k)* tm1 (i,plev)
      END DO
    
      DO k = 2, plev
         DO i = 1, plond
             Pbl_HgtLyI(i,k)=Pbl_HgtLyI(i,k-1)+con0(k)*tm1 (i,(plev+1-k))
         END DO
      END DO
...
!          xx1(i,kMax) =  qm(i,kMax) * ( MAX(phalf(i,kMax+1) - phalf(i,kMax),1.0_r8) )
  ...
  END SUBROUTINE tri_invert
   ...
   END MODULE Pbl_HostlagBoville_rna
