...
MODULE PblDriver
  ...
  USE Options, ONLY :  &
       nferr, nfprt,atmpbl,microphys,nClass
...
CONTAINS
  SUBROUTINE InitPBLDriver(ibMax,jbMax,kmax, sl, delsig, si)
    IMPLICIT NONE
    ...
    ELSE IF(atmpbl==3)THEN
       IF (microphys) THEN
          ...
          CALL vdinti(ibMax,jbMax,kmax,sl, delsig, si(1:kMax+1),pnats,pcnst)
       ELSE
          pnats=pnats_lsc
          pcnst=pcnst_lsc
          CALL vdinti(ibMax,jbMax,kmax,sl, delsig, si(1:kMax+1),pnats,pcnst)
  ...
  SUBROUTINE pbl_driver ( &
       ...
       LwCoolRate,LwCoolRateC,cldtot,cldinv,cldsat  ,cldson ,&
       qliq, bstar,&
       htdisp   ,taux    ,tauy    ,kt      ,jdt     , &
       PBL_CoefKm, PBL_CoefKh ,PBLSFC_CoefKm, PBLSFC_CoefKh, &
       pblh    ,tpert,qpert, tstar ,wstar,var,tauresx, &
       tauresy           ,gice, gliq ,gvar)
    IMPLICIT NONE
    ...
    REAL(KIND=r8) :: RQVBLTEN (nCols,kMax,3+nClass)
    ...
    real(KIND=r8) :: qm1     (nCols,kMax,3+nClass)
    ...
    REAL(KIND=r8) :: qpert_local(1:nCols,3+nClass)
    REAL(KIND=r8) :: cflx       (1:nCols,3+nClass)
    ...
    IF (microphys) THEN
       ...
       IF( nClass>0 .and. PRESENT(gvar))THEN
          CALL MellorYamada0 ( &
            ...
            tmtx  (1:ncols,1:kmax,1:3),qmtx  (1:ncols,1:kmax,1:5+nClass),umtx(1:ncols,1:kmax,1:4)  ,tmsfc (1:ncols,1:kmax,1:3)  ,qmsfc (1:ncols,1:kmax,1:5+nClass)   ,&
    ...
    ELSE IF( atmpbl == 4) THEN
       ...
       DO k=1,kMAx
          ...
          DO i=1,nCols    
             ...
             cldstratus(i,l)= cldinv(i,k)!total stratus cloud
       ...
       DO k=1,kMax+1
          DO i=1,nCols
             !kvm (i,kMax+2-k)= 0.0_r8 !PBL_CoefKm(i,k)
             !kvh (i,kMax+2-k)= 0.0_r8 !PBL_CoefKh(i,k)
       ...
       IF (microphys) THEN
          ...
          END IF
                ...
                cflx        (1:nCols,1:pcnst)      , &!REAL(r8), INTENT(in)    :: cflx       (pcols,ncnst) !real(r8), intent(in)    :: qflx(pcols) ! Water vapor flux at surface [ unit ? ]
  ...
  SUBROUTINE PblGridHistoryStorage(kt,jdt,latco,nCols,kMax,tmtx,qmtx,umtx)
   ...
   REAL(KIND=r8), INTENT(in   ) :: qmtx (ncols,kmax,5+nClass)
