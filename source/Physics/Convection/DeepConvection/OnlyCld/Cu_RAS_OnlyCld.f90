MODULE Cu_RAS_OnlyCld
  ...
  REAL(kind=r8),PARAMETER:: con_hvap   =2.5200e+6_r8      ! lat heat H2O cond   (J/kg)
...
!        END INTERFACE
  ...
  PUBLIC :: Init_Cu_RAS_OnlyCld
  PUBLIC :: Run_Cu_RAS_OnlyCld
  PUBLIC :: Finalize_Cu_RAS_OnlyCld
...
CONTAINS
  ...
  !-----------------------------------------------------------------------------------------
  SUBROUTINE Init_Cu_RAS_OnlyCld(kMax,dt,fhour,idate,iMax,jMax,ibMax,jbMax,si_in,sl_in)
    IMPLICIT NONE
  ...
  END SUBROUTINE Init_Cu_RAS_OnlyCld
  !------------------------------------------------------------
...
!  Generates random numbers in thread-safe mode.
  ...
  SUBROUTINE Run_Cu_RAS_OnlyCld(microphys,nClass,nAeros,nCols,latco,ntrac, kMax,dt,kdt,&
...
!  --- ...  array to track fraction of "cloud" in the form of ice
    ...
    DO k=1,kMax
       DO i=1, nCols
      !Kubota      IF(rain1(i) > 0.0_r8)THEN
      !Kubota        t3(i,k) = tgrs (i,k  ) !t3 (i,k) + (tgrs (i,k  ) - t2  (i,k))/(2*dt)! layer mean temperature ( k )K
      !Kubota        q3 (i,k)= qgrs (i,k,1)!q3 (i,k) + (qgrs (i,k,1) - q2  (i,k))/(2*dt)
      !Kubota        ql3(i,k)= qliq (i,k)!ql3(i,k) + (clw  (i,k,2) - qliq(i,k))/(2*dt)
      !Kubota        qi3(i,k)= qice (i,k)!qi3(i,k) + (clw  (i,k,1) - qice(i,k))/(2*dt)
      !Kubota        dudt(i,k) =(ugrs(i,k) - u2(i,k))/(2*dt)
      !Kubota        dvdt(i,k) =(vgrs(i,k) - v2(i,k))/(2*dt)      
      !Kubota      END IF   
        RAINCV(i) = 0.0_r8
       END DO      
    END DO
...
!      deallocate ( clw )
  ...
  END SUBROUTINE Run_Cu_RAS_OnlyCld



!Emarg: To allow OCL compute plcl and enable shallow
  ...
  SUBROUTINE InitMstad2()
   ...
   ! eps=gasr/rv
    ...
    DO i=1,the_ntemp
       ...
       !el=hl-(cl-con_cp)*(t-tbase)
       el=con_hvap-(cl-con_cp)*(t-tbase)
       esat = es3(t)
    ...
    DO i=1,fm_nthe
       ...
       DO k=2,fm_npres
          ...
          DO l=1,100
             ...
             !el=hl-(cl-con_cp)*(t-tbase)
             el=con_hvap-(cl-con_cp)*(t-tbase)
             power=ratio*el/(con_cp*t)
             pdkap=p-1.0e-2_r8*ess
             pdkap=kappa*LOG(pdkap)
             pdkap=EXP(pdkap)
             fun=t* EXP (power)/pdkap
             dfun=(fun/t)*(1.0e0_r8+(ratio/(con_cp*t) )*((con_cp-cl)*t &
  ...
  SUBROUTINE shllcl_oc(dt, ps, sl, si, qn1, tn1, kuo, &
    ...
    ! con_cp (cp)........Specific heat of air      (j/kg/k) 
    ! con_hvap(hl)........heat of evaporation of water     (j/kg) 
    ! con_rd (gasr)......gas constant of dry air        (j/kg/k)  
...
!                ENDIF
 ...
 REAL(kind=r8)  FUNCTION VTPF(ROR)
  ...
  !-----------------------------------------------------------------------------------------
  SUBROUTINE Finalize_Cu_RAS_OnlyCld()
    IMPLICIT NONE
    DEALLOCATE (rasal)

  END SUBROUTINE Finalize_Cu_RAS_OnlyCld
  !-----------------------------------------------------------------------------------------

END MODULE Cu_RAS_OnlyCld
