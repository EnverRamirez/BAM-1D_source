...
MODULE GridDynamics
  ...
  USE Options, ONLY:       &
      ...
      isimp             , &
      istrt,  & !Enver
      intfor, ndg, iffor, jdt, path_in, forcings_weight_d, forcings_weight_m, forcings_weight_t
...
!Enver additions 0 start
  ...
  PUBLIC :: GrpComp, Forcing_In, Forcings_Ascii, Forcings_Ascii_lsf, Nudging_Ascii, Geostrophy_Ascii
...
! Enver additions 1 start
  ...
  ELSEIF ( iffor == 0. ) THEN
    ...
    ndg = .FALSE.  !This line might be deleted when ndg option implemented
    ...
    ! guo, gvo, gto, gqo
     CALL Nudging_Ascii(guo,gvo,gto,gqo,ibmax,kmax,ppath,tod,jdt,xxday,colrad)
    !
     CALL Geostrophy_Ascii(gug,gvg,ibmax,kmax,ppath,tod,jdt,xxday,colrad)
    ENDIF
...
! LOGICAL, SAVE    :: OpenFile =.TRUE.
 ...
 IF( OpenFile_ndg )THEN    !OpenFile
     ...
     sondga=0.0; sondga2=0.0
      ...
      sigma_ndg(1)=1.0
      DO j=1,nlev-1
        sigma_ndg(j+1)= (sondg(j,1) - Pt)/(Ps - Pt) !Test (sond(j,1) - Pt)/(Ps - Pt) 
...
!      aumid(1:nlev)=aux2(1:nlev,6)         !umidade especifica. kg/kg
      ...
      sigma_ndg(1)=1.0
      DO j=1,nlev-1
        sigma_ndg(j+1)= (sondg2(j,1) - Pt)/(Ps - Pt) !Test (sond(j,1) - Pt)/(Ps - Pt) 
        !gps  (j)  = sigma_ndg(j)*(Ps - Pt) + Pt   !Test sigma(j)*(Ps - Pt) + Pt
      END DO   
      DO IL=1,nvars
